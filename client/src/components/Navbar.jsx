...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center px-4 pt-3">
      <div
        className={`w-full max-w-[1100px] transition-all duration-500 rounded-2xl border ${
          scrolled
        ...
        <div className="px-5 sm:px-6">
          <div className="flex items-center justify-between h-14">
            ...
            <Link to={user ? '/dashboard' : '/'} className="flex items-center gap-2 group shrink-0">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#7c3aed] to-[#ec4899] flex items-center justify-center text-white font-bold text-xs shadow-md shadow-[#7c3aed]/25 group-hover:shadow-[#7c3aed]/50 transition-all duration-300 group-hover:scale-105">
                C
              </div>
              <span className="text-lg font-bold gradient-text hidden sm:inline">CareerPath</span>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-0.5">
              {navLinks.map((link) => (
                ...
                >
                  {link.label}
                  {isActive(link.to) && (
                    <span className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 w-4 h-[2px] bg-gradient-to-r from-[#7c3aed] to-[#ec4899] rounded-full" />
                  )}
                </Link>
              ))}
            </div>
            ...
            <div className="hidden md:flex items-center gap-2 shrink-0">
              {user ? (
                <>
                  <button className="relative p-2 text-gray-400 hover:text-white rounded-lg hover:bg-white/[0.06] transition-all duration-200">
                    <Bell size={17} />
                    <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-[#ec4899] rounded-full ring-2 ring-[#0d0d24]" />
                  </button>
                  <div className="relative" ref={menuRef}>
                    ...
                    >
                      <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-[#7c3aed] to-[#ec4899] flex items-center justify-center">
                        <span className="text-white text-[10px] font-bold">{user.name?.[0]?.toUpperCase()}</span>
                      </div>
                      <span className="text-xs font-medium text-gray-300 max-w-[80px] truncate">{user.name}</span>
                ...
                <div className="flex items-center gap-2">
                  <Link
                    to="/login"
                    className="px-3.5 py-1.5 text-[13px] font-medium text-gray-300 hover:text-white rounded-lg hover:bg-white/[0.06] transition-all duration-200"
                  >
                    Sign In
                  </Link>
                  <Link
                    to="/register"
                    className="px-4 py-1.5 text-[13px] font-semibold bg-gradient-to-r from-[#10b981] to-[#059669] rounded-full text-white shadow-md shadow-emerald-500/20 hover:shadow-emerald-500/40 hover:scale-[1.03] transition-all duration-200 flex items-center gap-1.5"
                  >
